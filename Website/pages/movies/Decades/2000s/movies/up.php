<!DOCTYPE html>
<html lang="en">
<head>
  <title>Movie Blog</title>
  <meta charset="utf-8"/>
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <link rel="stylesheet" href="../../style.css">
</head>

<script src="app.js"></script>

<style></style>


<div class="navbar">
	<a href="../../../../../index.php">Home</a>
	<a href="../../../../recipes/recipes.php">Food</a>
	<a href="../../../250.php">Movies</a>
	<a href= "../../../../portfolio/portfolio.php">Portfolio</a>
	<a href= "../../index.php">Travel</a>
  <a href= "../../index.php">Contact Me</a>
</div>



<body>
  <div id="page">

    <!-- MAIN CONTENT-->
    <div id="content" class="side">
      <div class="header">
        <div class="top">
          <div id="title" class="title">
            <h1> Up (2009)</h1>
          </div>
          <div id="rating" class="title">
            <h2 class="single">10</h2>
          </div>
        </div>
        <div class="mid">
          <div id="picture" class="info">
            <img src="../images/up.jpg">
          </div>
          <div id="synopsis" class="info">
            <p class="synopsis"> <br><br>78-year-old Carl Fredricksen travels to Paradise Falls in his house equipped with balloons, inadvertently taking a young stowaway.</p>
            <p id="info">Director: Pete Docter</p>
            <p id="info">Writer: Pete Docter</p>
            <p id="info">Actors: Edward Asner, Jordan Nagai, John Ratzenberger<br><br><br><br><br><br><br><br><br><br></p>
  
          </div>
        </div>
      </div>
  
      <div id="review">
          <p>
            Upon initial release Up was met with insane critical and commercial success, cementing itself as one of The 
            Greats within Pixar's, at the time, spotless resume of classics. Of course since 2009 Pixar has gone from consistently releasing
            films that pioneer and reinvent the Animated genre to putting out far less innovative works that are often sequels to prior
            franchises. These works, while never the bottom of the barrel of animated entertainment, are far from the impressive heights
            that Pixar was previously attaining on a yearly basis. This tainting of Pixar's good name has shown doubt on even their
            greatest hits as edgy film critics claim Pixar was "never that good." Apart of these cynics' opinions is often that the
            only reason Up is good is because of the opening ten minutes. I am here to convince you that these people could not be more wrong.
            <br><br>

            
          </p>
          
      </div>


    </div>
    
    <!-- SIDEBAR -->
    <div id="sidebar" class="side">
      <div id="sideHeader">
        <a href="../../../250.php">
          <img style="width: 250px; height: 200px;" src="../../../images/Nav/top250.jpg">
        </a>
        <a href="../../1900s/1900s.php">
          <img style="width: 250px; height: 55px;" src="../../../images/Nav/1900s.jpg">
        </a>
        <a href="../../1930s/1930s.php">
          <img style="width: 250px; height: 65px;" src="../../../images/Nav/1930s.jpg">
        </a>
        <a href="../../1940s/1940s.php">
          <img style="width: 250px; height: 55px;" src="../../../images/Nav/1940s.jpg">
        </a>
        <a href="../../1950s/1950s.php">
          <img style="width: 250px; height: 65px;" src="../../../images/Nav/1950s.jpg">
        </a>
        <a href="../../1960s/1960s.php">
          <img style="width: 250px; height: 65px;" src="../../../images/Nav/1960s.jpg">
        </a>
        <a href="../../1970s/1970s.php">
          <img style="width: 250px; height: 65px;" src="../../../images/Nav/1970s.jpg">
        </a>
        <a href="../../1980s/1980s.php">
          <img style="width: 250px; height: 65px;" src="../../../images/Nav/1980s.jpg">
        </a>
        <a href="../../1990s/1990s.php">
          <img style="width: 250px; height: 65px;" src="../../../images/Nav/1990s.jpg">
        </a>
        <a href="../../2000s/2000s.php">
          <img style="width: 250px; height: 65px;" src="../../../images/Nav/2000s.jpg">
        </a>
        <a href="../../2010s/2010s.php">
          <img style="width: 250px; height: 65px;" src="../../../images/Nav/2010s.jpg">
        </a>
        <a href="../../2020s/2020s.php">
          <img style="width: 250px; height: 65px;" src="../../../images/Nav/2020s.jpg">
        </a>



      </div>


    </div>



  </div>


  
  

	
	
	
	
  <div class="footer" >
	  <p>Build by Christian Arbelaez for his online portfolio. Copyright © by Christian Arbelaez. You are allowed to use this webpage for both personal or commercial use.</p>
	</div>
  
	
  
  
  
  


</body>
</html>


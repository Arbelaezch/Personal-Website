<!DOCTYPE html>
<html lang="en">
<head>
  <title>2010s</title>
  <meta charset="utf-8"/>
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <link rel="stylesheet" href="../Dstyle.css">
</head>


<body>
  <?php echo require_once("../../../../nav.php"); ?>


  <article class="center">
    <!-- MAIN CONTENT-->
    <div class="content">
        <header>
            <h1 class="title">Contemporary Cinema (1995 - Today)</h1>
        </header>
        <div id="description" class="mid">
          <p> 
            <img src="../../images/construction.jpg"/>
				<br>
				
        
            <br><br>

            
            <br><br>

            Fun Fact: 

            <br><br>
            Favorites of the Decade:  
          </p>
        </div>
        <div id="grid">
            <div class="wrapper">
              <div class="item">
                <a href="./movies/thing2.php">
                    <img src="./images/thing2.jpg"></img>
                    <p>The Thing (2011)</p>
                </a>
              </div>
              <div class="item">
                <a href="./movies/gravity.php">
                    <img src="./images/gravity.jpg"></img>
                    <p>Gravity (2013)</p>
                </a>
              </div>
              <div class="item">
                <a href="./movies/interstellar.php">
                    <img src="./images/interstellar.jpg"></img>
                    <p>Interstellar (2014)</p>
                </a>
              </div>
              <div class="item">
                <a href="./movies/spectre.php">
                    <img src="./images/spectre.jpg"></img>
                    <p>Spectre (2015)</p>
                </a>
            </div>
              <div class="item">
                  <a href="./movies/busan.php">
                      <img src="./images/busan.jpg"></img>
                      <p>Train to Busan (2016)</p>
                  </a>
              </div>
              <div class="item">
                <a href="./movies/accountant.php">
                    <img src="./images/accountant.png"></img>
                    <p>The Accountant (2016)</p>
                </a>
              </div>
              <div class="item">
                <a href="./movies/independenceDay2.php">
                    <img src="./images/independenceDay.jpg"></img>
                    <p>Independence Day: Resurgence (2016)</p>
                </a>
              </div>
              <div class="item">
                <a href="./movies/deathOfStalin.php">
                    <img src="./images/deathStalin.jpg"></img>
                    <p>The Death of Stalin (2017)</p>
                </a>
              </div>
              <div class="item">
                <a href="./movies/kong.php">
                    <img src="./images/Kong_Skull_Island_poster.jpg"></img>
                    <p>Kong: Skull Island (2017)</p>
                </a>
              </div>
              <div class="item">
                <a href="./movies/sistersBrothers.php">
                    <img src="./images/sistersBrothers.jpg"></img>
                    <p>The Sisters Brothers (2018)</p>
                </a>
              </div>
              <div class="item">
                <a href="./movies/godzilla2.php">
                    <img src="./images/godzilla2.jpg"></img>
                    <p>Godzilla: King of the Monsters (2019)</p>
                </a>
              </div>
              <div>
                <a href="./movies/favourite.php">
                    <img src="./images/favourite.jpeg"></img>
                    <p>The Favourite (2018)</p>
                </a>
              </div>
              <div>
                <a href="./movies/king.php">
                    <img src="./images/king.jpg"></img>
                    <p>The King (2019)</p>
                </a>
              </div>

            </div>  
        </div>
    </div>
      
      <!-- SIDEBAR -->
      <div id="sidebar">
          <div id="sideHeader">
              <a href="../../250.php">
                  <img style="width: 250px; height: 200px;" src="../../images/Nav/top250.jpg" alt="top 250 movies">
                </a>
                <a href="../1900s/1900s.php">
                  <img style="width: 250px; height: 65px;" src="../../images/Nav/1900s.jpg" alt="1900s">
                </a>
                <a href="../1930s/1930s.php">
                  <img style="width: 250px; height: 65px;" src="../../images/Nav/1930s.jpg" alt="1930s">
                </a>
                <a href="../1940s/1940s.php">
                  <img style="width: 250px; height: 55px;" src="../../images/Nav/1940s.jpg" alt="1940s">
                </a>
                <a href="../1950s/1950s.php">
                  <img style="width: 250px; height: 65px;" src="../../images/Nav/1950s.jpg" alt="1950s">
                </a>
                <a href="../1960s/1960s.php">
                  <img style="width: 250px; height: 65px;" src="../../images/Nav/1960s.jpg" alt="1960s">
                </a>
                <a href="../1970s/1970s.php">
                  <img style="width: 250px; height: 65px;" src="../../images/Nav/1970s.jpg" alt="1970s">
                </a>
                <a href="../1980s/1980s.php">
                  <img style="width: 250px; height: 65px;" src="../../images/Nav/1980s.jpg" alt="1980s">
                </a>
                <a href="../1990s/1990s.php">
                  <img style="width: 250px; height: 65px;" src="../../images/Nav/1990s.jpg" alt="1990s">
                </a>
                <a href="../2000s/2000s.php">
                  <img style="width: 250px; height: 65px;" src="../../images/Nav/2000s.jpg" alt="2000s">
                </a>
                <a href="../2010s/2010s.php">
                  <img style="width: 250px; height: 65px;" src="../../images/Nav/2010s.jpg" alt="2010s">
                </a>
                <a href="../2020s/2020s.php">
                  <img style="width: 250px; height: 65px;" src="../../images/Nav/2020s.jpg" alt="2020s">
                </a>
          </div>
      </div>
  </article>


  <footer>
	  <p>Build by Christian Arbelaez for his online portfolio. Copyright © by Christian Arbelaez. You are allowed to use this webpage for both personal or commercial use.</p>
	</footer>
  
	
  
  
  
  


</body>
</html>